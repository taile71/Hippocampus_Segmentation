...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    #Saggittal Overlay View
    sag_img = np.flip((orig_vol[slice_nums[0],:,:]/np.max(orig_vol))*0xff).T.astype(np.uint8)
    sag_shape = np.flip(8*np.array(sag_img.shape), 0)
    sag_img_conv = Image.fromarray(sag_img, mode="L").convert("RGBA").resize(sag_shape)
    
    sag_pred_img = np.flip((pred_vol[slice_nums[0],:,:]/np.max(pred_vol))*0xff).T.astype(np.uint8)
    sag_pred_img_conv = Image.fromarray(sag_pred_img, mode="L").convert("RGBA").resize(sag_shape)
    sag_mask = Image.new('RGBA',sag_pred_img_conv.size,(0,0,0,100))
    sag_overlay = Image.composite(sag_img_conv , sag_pred_img_conv,sag_mask)
    draw.text((80, 240), f"Sagittal slice #{slice_nums[0]}", (255, 255, 255), font=main_font)
    pimg.paste(sag_overlay, box=(50, 280))
    
    #Coronal Overlay View
    cor_img = np.flip((orig_vol[:,slice_nums[1],:]/np.max(orig_vol))*0xff).T.astype(np.uint8)
    cor_shape = np.flip(8*np.array(cor_img.shape), 0)
    cor_img_conv = Image.fromarray(cor_img, mode="L").convert("RGBA").resize(cor_shape)
    
    cor_pred_img = np.flip((pred_vol[:,slice_nums[1],:]/np.max(pred_vol))*0xff).T.astype(np.uint8)
    cor_pred_img_conv = Image.fromarray(cor_pred_img, mode="L").convert("RGBA").resize(cor_shape)
    cor_mask = Image.new('RGBA',cor_pred_img_conv.size,(0,0,0,100))
    cor_overlay = Image.composite(cor_img_conv, cor_pred_img_conv,cor_mask)
    draw.text((100+sag_shape[0], 240), f"Coronal slice #{slice_nums[1]}", (255, 255, 255), font=main_font)
    pimg.paste(cor_overlay, box=(100+sag_shape[0], 280))

    #Axial Overay View
    axl_img = np.flip((orig_vol[:,:,slice_nums[2]]/np.max(orig_vol))*0xff).T.astype(np.uint8)
    axl_shape = np.flip(8*np.array(axl_img.shape), 0)
    axl_img_conv = Image.fromarray(axl_img, mode="L").convert("RGBA").resize(axl_shape)
    
    axl_pred_img = np.flip((pred_vol[:,:,slice_nums[2]]/np.max(pred_vol))*0xff).T.astype(np.uint8)
    axl_pred_img_conv = Image.fromarray(axl_pred_img, mode="L").convert("RGBA").resize(axl_shape)
    axl_mask = Image.new('RGBA',axl_pred_img_conv.size,(0,0,0,100))
    axl_overlay = Image.composite(axl_img_conv, axl_pred_img_conv,axl_mask)
    draw.text((130+cor_shape[0]+sag_shape[0], 240), f"Axial slice #{slice_nums[2]}", (255, 255, 255), font=main_font)
    pimg.paste(axl_overlay, box=(130+cor_shape[0]+sag_shape[0], 280))
...
def get_series_for_inference(path):
    ...
    series_for_inference = [file for file in dicoms if file.SeriesDescription == 'HippoCrop']
...
if __name__ == "__main__":
    ...
    study_dir = [hip_dir for hip_dir in subdirs if 'Crop' in hip_dir][0]
    ...
    inference_agent = UNetInferenceAgent(
        device="cpu",
        parameter_file_path=r"/home/workspace/src/inference/model.pth")
    ...
    report_save_path = r"/home/workspace/out/report10.dcm"
